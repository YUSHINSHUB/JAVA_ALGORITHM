...
public class Q1107_Remote_Controller_G5 {
	...
	public static void main(String[] args) throws IOException {
		...
		// Using HashMap to track the functionality of buttons, 1 if it's working and 0 otherwise.
		...
		// Initialize the result with the difference between the current channel (100) and the desired channel.
		int res;
		if (n > 100)
			res = n - 100;
		else
		...
		for (int i = 0; i <= 1000000; i++) {
			...
			for (int j = 0; j < t.length(); j++) {
				...
				// If we reach the end of the channel number without encountering a broken button.
				if (j == t.length() - 1) {
					// Update the result with the minimum between the current result and the sum of the length of the channel number and the difference between the desired and current channel.
