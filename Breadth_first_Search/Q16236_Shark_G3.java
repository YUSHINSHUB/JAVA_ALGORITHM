...
This Java code implements a grid-based simulation in which an entity moves and consumes items. The entity increases its size after consuming a certain number of items smaller than itself. A breadth-first search (BFS) is used to navigate the grid and find the nearest consumable item. The process is repeated until no consumable items remain, and the total distance traveled is calculated.
이 Java 코드는 그리드 기반의 시뮬레이션을 구현합니다. 여기서 한 객체가 이동하며 크기가 작은 아이템을 소비합니다. 객체는 일정 수의 아이템을 소비한 후 크기가 증가하며, 너비 우선 탐색(BFS)을 사용하여 그리드를 탐색하고 가장 가까운 소비 가능한 아이템을 찾습니다. 소비 가능한 아이템이 없을 때까지 이 과정이 반복되며, 이동한 총 거리가 계산됩니다.
*/

import java.io.*;
import java.util.*;

public class Q16236_Shark_G3 {
